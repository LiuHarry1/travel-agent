    ...
    <div className="app">
      ...
      <header className={`top-bar ${sidebarVisible ? 'with-sidebar' : 'no-sidebar'}`}>
        <div className="top-bar-left">
          ...
          {activeTab === 'chat' && !sidebarVisible && (
            <button type="button" className="new-chat-btn-top" onClick={createSession}>
              <svg
                width="18"
                height="18"
                viewBox="0 0 20 20"
      ...
      <aside className={`sidebar ${sidebarVisible ? 'visible' : 'hidden'}`}>
        <div className="sidebar-header">
          <div className="sidebar-brand">
            ...
            <div className="brand-name">MRT QA Assistant</div>
          </div>
          <button
            type="button"
        ...
        <nav className="sidebar-nav">
          <button
            ...
            onClick={() => {
              // Always create a brand new empty chat
              createSession()
              // Ensure we are on chat tab so the new empty chat is visible
              if (activeTab !== 'chat') {
                onTabChange('chat')
              }
            }}
        ...
        <div className="sidebar-sessions">
          ...
          <div className="sidebar-sessions-list">
            {sessions.filter((s) => s.history.length > 0).length === 0 ? (
